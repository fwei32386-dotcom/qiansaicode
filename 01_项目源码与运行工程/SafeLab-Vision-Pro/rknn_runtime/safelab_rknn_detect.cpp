...
struct Options {
    ...
    std::string label_preset = "legacy7";
...
void print_usage() {
    std::cout
        ...
        << "  safelab_rknn_detect --model model.rknn --image frame.rgb --frame-width 1280 --frame-height 720 [--label-preset legacy7|ppe|fire_smoke] [--labels a,b,c] [--input-type uint8|int8] [--pass-through] [--conf-threshold 0.25] [--dump-output-stats] [--output detections.jsonl]\n"
        << "\n"
...
std::vector<std::string> labels_for_preset(const std::string& preset) {
    if (preset == "legacy7") {
        return safelab::labels();
    }
    if (preset == "ppe") {
        return {"person", "helmet", "vest", "goggles", "gloves"};
    }
    if (preset == "fire_smoke") {
        return {"fire", "smoke"};
    }
    throw std::runtime_error("--label-preset expects legacy7, ppe, or fire_smoke");
}

std::vector<std::string> active_labels_for(const Options& options) {
    if (!options.label_names.empty()) {
        return options.label_names;
    }
    return labels_for_preset(options.label_preset);
}
...
Options parse_args(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        } else if (arg == "--label-preset") {
            options.label_preset = require_value(arg);
            labels_for_preset(options.label_preset);
        } else if (arg == "--labels") {
            options.label_names = split_csv(require_value(arg));
            if (options.label_names.empty()) {
                throw std::runtime_error("--labels requires at least one label");
            }
...
std::vector<safelab::RawDetection> run_rknn_detection(const Options& options) {
    ...
    const std::vector<std::string> label_names = active_labels_for(options);
    const int channels = static_cast<int>(label_names.size()) + 4;
    ...
    if (anchors <= 0 || output_attr.n_elems != static_cast<uint32_t>(channels * anchors)) {
        ...
        error << "RKNN output element count cannot be decoded as YOLOv8 ["
              << channels << ",anchors]; labels=" << label_names.size()
              << " n_elems=" << output_attr.n_elems;
