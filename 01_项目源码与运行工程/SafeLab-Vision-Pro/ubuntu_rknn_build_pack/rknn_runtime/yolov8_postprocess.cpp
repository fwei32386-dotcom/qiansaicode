...
std::vector<RawDetection> decode_yolov8_channel_major(
    ...
    if (channels != static_cast<int>(labels().size()) + 4) {
        throw std::runtime_error("unexpected YOLOv8 channel count");
    }
...
Detection to_detection(
    ...
    int frame_width,
    int frame_height) {
    if (raw.class_id < 0 || raw.class_id >= static_cast<int>(labels().size())) {
    ...
    detection.class_name = labels()[raw.class_id];
